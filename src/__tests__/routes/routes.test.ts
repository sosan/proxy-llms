...
describe('registerRoutes', () => {
  it('should register all expected routes', () => {
    ...
    expect(app.post).toHaveBeenCalledWith('/:version/messages/count_tokens', expect.any(Function))
    ...
    // HEAD / OPTIONS probes
    expect(app.on).toHaveBeenCalledWith('HEAD,OPTIONS', '/:version/messages', expect.any(Function))
    expect(app.on).toHaveBeenCalledWith('HEAD,OPTIONS', '/:version/messages/count_tokens', expect.any(Function))
  ...
  it('should not register any unexpected routes', () => {
    ...
    const onPaths = app.on.mock.calls.map((call: any[]) => call[1])
    ...
    expect(onPaths).toContain('/:version/messages')
    expect(onPaths).toContain('/health')
  })
})
