...
import { ProviderConfigs, resolveAnthropicModel } from '../config/providers'
...
export const handleClaudeMessages = async (c: Context<{ Bindings: Env }>) => {
  ...
  try {
    ...
    const modelName = modelParts.length > 1 ? modelParts.pop()! : mappedModel
    ...
    // -- 5. Resolve provider instance & make request ------------------------
...
function handleClaudeError(
  ...
  const publicMessage = error instanceof ProviderError
    ? error.publicMessage
